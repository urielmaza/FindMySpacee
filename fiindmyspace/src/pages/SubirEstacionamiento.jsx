...
const SubirEstacionamiento = () => {
  ...
  const handleGuardarMapa = async () => {
    ...
    // Construir tarifas a partir de vehiculos y modalidades (parseadas a número)
    const tarifasPayload = [];
    if (tipo === 'privado') {
      for (const v of vehiculos) {
        for (const modalKey of modalidades) {
          const raw = v.precios?.[modalKey];
          if (raw === undefined || raw === null || raw === '') {
            setMensaje('Error: Debes completar todos los precios para las modalidades seleccionadas.');
            setTipoMensaje('error');
            return;
          }
          const parsed = Number(raw);
          if (Number.isNaN(parsed) || parsed < 0) {
            setMensaje('Error: Algunos precios ingresados no son válidos. Usá sólo números.');
            setTipoMensaje('error');
            return;
          }
          tarifasPayload.push({ tipo_vehiculo: v.nombre || 'Desconocido', modalidad: modalKey, precio: parsed });
        }
      }
    }
    const body = {
